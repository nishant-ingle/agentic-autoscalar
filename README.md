...
pka-autoscaler/
├── README.md                   # Your detailed design doc goes here
├── Makefile                    # Root Makefile with targets like `make build`, `make deploy`
├── .github/                    
│   └── workflows/              # CI/CD pipelines for testing and linting
│
├── controller/                 # 🏗️ The Go Kubernetes Operator
│   ├── api/                    # Go structs defining your Custom Resource Definition (CRD)
│   ├── cmd/                    # Entrypoint (main.go) for the operator manager
│   ├── config/                 # Kustomize manifests auto-generated by Kubebuilder
│   ├── internal/               # Private code (standard Go layout practice)
│   │   ├── controller/         # The Reconcile loop (the core logic talking to K8s API)
│   │   └── client/             # HTTP client to talk to the Python Inference API
│   ├── Dockerfile              # Multi-stage Docker build for the Go binary
│   ├── go.mod
│   └── go.sum
│
├── inference-engine/           # 🧠 The Python AI/ML Service
│   ├── app/
│   │   ├── __init__.py
│   │   ├── main.py             # FastAPI application initialization
│   │   ├── api/                # REST endpoints (e.g., routes.py containing /api/v1/predict)
│   │   ├── ml/                 # PyTorch models, forecasting logic, and data processin
│   │   └── core/               # Configuration, logging, and error handling (Circuit Breakers)
│   ├── tests/                  # Pytest suite for the API and ML models
│   ├── Dockerfile              # Docker build for the Python service
│   └── requirements.txt        # (Or pyproject.toml if using Poetry/uv)
│
├── deploy/                     # 🚀 Deployment & Infrastructure
│   ├── helm/                   # Helm charts to deploy both components and the CRD together
│   └── prometheus/             # Example Prometheus scraping configs for testing
│
└── hack/                       # 🛠️ Local Development Scripts
    └── setup-k3d.sh            # Script to spin up a local K3D cluster for testing
